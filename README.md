...
flowchart TD
    ...
    subgraph ORCH["Orchestration · one core, three wrappers — 🟡 Written (structurally tested, not live-run)"]
        direction LR
...
> Status: the ML pipeline (ETL → serving) is built and tested, and all three orchestrators (Airflow/Prefect/Kubeflow) are implemented and structurally tested (🟡 _Written_ — not yet validated on live runtimes). CI (ruff + pytest) is written and runs on first push. The 7-step pipeline path has no `NotImplementedError`; only the optional `run_pipeline()`/CLI convenience entry points remain stubs. See the Roadmap for per-stage status.
...
**ML pipeline (built):** ETL → features → models (sklearn + PyTorch) → MLflow registry → BentoML `/predict`, end to end on synthetic data. **Orchestrators (built):** the same `pipeline_tasks` core is wrapped by an Airflow DAG, a Prefect flow, and a Kubeflow KFP pipeline — all implemented and structurally tested, with the trade-off write-up in [`docs/orchestrator-comparison.md`](docs/orchestrator-comparison.md). **Next:** validate the three wrappers on live runtimes (Airflow stack / Prefect server / `kind` cluster) and a hosted CI run on first push.
